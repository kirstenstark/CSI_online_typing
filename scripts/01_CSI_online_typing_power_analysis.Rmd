...
glmm1 <- glmer(RT ~ PosOr + (1|subject) +(PosOr|category) ,
             data = model_df, family =Gamma(link ="identity"), control=glmerControl(optimizer = "bobyqa"), nAGQ=20)
isSingular(glmm_red)
# model converges so we use this
summary(glmm_red)
```
...
```{r}
set.seed(27)
#doTest(lmm1_log, fixed("PosOr.L"))
powerSim(lmm1_log, test=fixed("PosOr.L","t"), nsim = 50) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
```
# 5) Power analyses at a range of sample sizes
```{r}
pc1 <- powerCurve(lmm1_log, breaks = c(10, 20, 25, 30, 35, 40),
                  nsim = 3) # increase nsim for real power analysis
```
```{r}
pc1
```
...
lmm1 <- lmer(RT ~ PosOr.L + (1|subject) +(PosOr.L|category) , 
