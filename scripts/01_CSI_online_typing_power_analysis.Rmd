...
lmm1 <- lmer(RT ~ PosOr.L + (PosOr.L|subject) +(1|category) , 
            data =model_df, REML = FALSE)
summary(lmm1)

# Model 2: Linear model with log transformed RT data
lmm1_log <- lmer(lRT ~ PosOr.L + (PosOr.L|subject) +(1|category) , 
            data =model_df, REML = FALSE)
summary(lmm1_log)

# Model 3: Generalized linear model with gamma distribution
glmm1 <- glmer(RT ~ PosOr.L+ (PosOr|subject) +(PosOr.L|category) , 
...
glmm1 <- glmer(RT ~ PosOr + (1|subject) +(PosOr|category) ,
...
#2) Extend dataset
```{r}
lmm1 <- extend(lmm1, along="subject", n=80)
m2data <- getData(lmm1) 
## ok, data were indeed extended to n subjects ;-)
str(m2data)
str(model_df)

lmm1_log <- extend(lmm1_log, along="subject", n=80)
m2data <- getData(lmm1_log) 
## ok, data were indeed extended to n subjects ;-)
str(m2data)
str(model_df)

glmm1<- extend(glmm1, along="subject", n=80)
...
#3) Specify effect size: 15 min (as in the online experiment)
...
```{r}
fixef(lmm1)
fixef(lmm1)["PosOr.L"] <- 15

fixef(lmm1_log)
fixef(lmm1_log)["PosOr.L"] <- 0.01

fixef(glmm1)
fixef(glmm1)["PosOr.L"] <- 15
```
...
powerSim(lmm1, test=fixed("PosOr.L","t"), nsim = 1000) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
```
2) Log linear model
...
powerSim(glmm1, test=fixed("PosOr.L","t"), nsim = 1000) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
```
...
Unconverted RTs
```{r}
pc1_linear <- powerCurve(lmm1, breaks = c(30, 35, 40, 50, 60, 70),
