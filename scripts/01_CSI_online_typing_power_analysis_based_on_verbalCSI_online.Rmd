...
# load data
load(here::here("data_for_power_analysis","df.RData"))
df_online <- df
...
```{r load data lab}
# load data 
load(here::here("data_for_power_analysis","CSI_model_df.RData"))
df_lab <- model_df
...
```{r PowerSim LMM1}
(PowerLMM1 <- powerSim(lmm1, test=fixed("Pos.L","t"), nsim = 1000)) # increase to nsim > 1000 for real test
lastResult()$warnings
lastResult()$errors
```
