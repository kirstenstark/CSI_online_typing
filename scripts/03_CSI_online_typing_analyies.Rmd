...
answers <- answers %>% 
  filter(word != "ARMAND", # Item = Armband
                               ...
                               word != "KBACKSPACEZTORTE",# Item = KUCHEN
...
# These 41 entries will be considered as incorrect 
(answers)
...
for (i in 1:nrow(answers)) {
  df_red$word[df_red$subject == answers$df.subject[i] & 
            df_red$item == tolower(answers$item[i])] <- NA
}
sum(is.na(df_red$word)) # did it work
```
...
# Unvalid trials: Ordinal Position changed 
```{r}
# update ordinal position number
# add position number
df_red$PosOr_correct <- as.numeric(as.character(df$PosOr))
for(i in 1:nrow(df_red)) {
  if(is.na(df_red$word[i])){
    df_red$PosOr_correct[i] <- df_red$PosOr_correct[i] -1
  }
  if(df_red$PosOr_correct[i] == 0) {
    df_red$PosOr_correct[i] = NA
    }  }
table(df_red$PosOr_correct)

# convert variables to factors
is.numeric(df_red$timing.01)
df_red$PosOr_correct <- as.factor(df_red$PosOr_correct)
df_red$subject <- as.factor(df_red$subject)
# descriptives 
(means_redPosOr <- Rmisc::summarySEwithin(df_red,"timing.01",idvar = "subject",withinvars = "PosOr_correct", na.rm = T))
...
```{r}
(plot <- ggplot(data=df_red[!is.na(df_red$PosOr_correct),], aes(x=PosOr_correct, y=timing.01)) +
  stat_summary(fun=mean,  geom="point", size = 2)+
  stat_summary(fun=mean,  geom="line", size = 1) +
  apatheme+
  labs(x="Ordinal Position ",y ="RT ms")+
...
(plot_by_cat <- ggplot(data=df_red[!is.na(df_red$PosOr_correct),], aes(x=PosOr_correct, y=timing.01)) +
  stat_summary(fun=mean,  geom="point", size = 2)+
  stat_summary(fun=mean,  geom="line", size = 1) +
  facet_wrap(~category) +
  apatheme+
...
# Unvalid trials: Ordinal Position of unvalid trials set to NA
```{r}
...
is.numeric(df_red$timing.01)
df_red$PosOr <- as.factor(df_red$PosOr)
df_red$subject <- as.factor(df_red$subject)
# descriptives 
means_unvalidtoNA <- Rmisc::summarySEwithin(df_red,"timing.01",idvar = "subject",withinvars = "PosOr", na.rm = T)
...
df_red$subject <- as.factor(df_red$subject)
# descriptives 
(means_PosOrNA <- Rmisc::summarySEwithin(df,"timing.01",idvar = "subject",withinvars = "PosOr", na.rm = T))
```


```{r}
(plot <- ggplot(data=df_red[!is.na(df_red$PosOr),], aes(x=PosOr, y=timing.01)) +
  stat_summary(fun=mean,  geom="point", size = 2)+
  stat_summary(fun=mean,  geom="line", size = 1) +
  apatheme+
  labs(x="Ordinal Position ",y ="RT ms")+
...
(plot_by_cat <- ggplot(data=df_red[!is.na(df_red$PosOr),], aes(x=PosOr, y=timing.01)) +
  stat_summary(fun=mean,  geom="point", size = 2)+
  stat_summary(fun=mean,  geom="line", size = 1) +
  facet_wrap(~category) +
  apatheme+
...
```{r}
hist(df_red$timing.01)

contrasts(df_red$PosOr) <- contr.poly(5)

m1 <- glmer(timing.01 ~ PosOr + (PosOr|subject) +(PosOr|category) , 
             data =df_red, family =Gamma(link ="identity"), control=glmerControl(optimizer = "bobyqa"))
