...
cowplot::plot_grid(plot_rt, boxplot,
  ...
  ggsave(filename = here::here("figures",
         ...
         device = cairo_pdf)
#embedFonts(file = here::here("figures", "CSI_online_typing_RTs_and_normalized_RTs"))
...
Boxplot with predicted regression line

```{r boxplot_with regression_line}
...
```{r plot_rt_norm_by_participant_mediansplit}
# modeloutput <- coef(m1)$subject
# (means_final_subject <- df_valid %>% 
#    summarySEwithin(.,"timing.01",withinvars = c("subject","PosOr")))
# (means_final<- df_valid %>%  
...
#   means_final_subject$normalizedRT[i] <- means_final_subject$timing.01[i] -
#     means_final_subject$timing.01[means_final_subject$subject == means_final_subject$subject[i] & means_final_subject$PosOr == 1]
#   # prepare for median split
#   means_final_subject$effect[i] <- modeloutput$PosOr.cont[means_final_subject$subject[i]] 
# #means_final_subject$subjectmean[i] <- mean(df_valid$timing.01[df_valid$subject == means_final_subject$subject[i]])
# }
# # median split 
# estimate <- summary(m1)
# for(i in 1:nrow(means_final_subject)) {
#   if (means_final_subject$effect[i] <
#       estimate$coefficients[2,1]) {
#     means_final_subject$mediansplit[i] <- "lower"
#   } else if (means_final_subject$effect[i] >
#                estimate$coefficients[2,1]) {
#     means_final_subject$mediansplit[i] <- "higher"
#   } else {
#     means_final_subject$mediansplit[i] <- "median"
#     print("onmedian")
#   
#   # if (means_final_subject$subjectmean[i] <
#   #     median(means_final_subject$subjectmean)) {
#   #   means_final_subject$mediansplit[i] <- "low"
#   # } else if (means_final_subject$subjectmean[i] >
#   #              median(means_final_subject$subjectmean)) {
#   #   means_final_subject$mediansplit[i] <- "high"  
#   # } else {
#   #   means_final_subject$mediansplit[i] <- "median" 
#   #   print("onmedian")
# }}
...
# #     geom_point(size = 2, aes(group=subject, color=mediansplit, linetype=mediansplit)) +
# #     geom_line(size = 1, aes(group=subject, color = mediansplit, linetype=mediansplit)) +
# #     geom_point(aes(x=PosOr,y=grandmean), size = 4)+
# #     geom_line(aes(x=PosOr,y=grandmean), group = 1,size = 1)+
# #     #geom_errorbar(aes(ymin=timing.01-se, ymax=timing.01+se), width =.1) +
# #     apatheme+
...
# #   #         color="black", size = 8))
# 
# median <- median(means_final_subject$effect)
# (plot_rt <- means_final_subject %>% 
#     ggplot(., aes(x=PosOr,y=normalizedRT)) +
#     geom_point(size =1, aes(color = mediansplit)) +
#     geom_line(size = 1, aes(group =1, color = mediansplit)) +
#     geom_point(aes(x=PosOr,y=grandmean), size = 1)+
#     geom_line(aes(x=PosOr,y=grandmean), group = 1,size = 1)+
#     #geom_errorbar(aes(ymin=timing.01-se, ymax=timing.01+se), width =.1) +
#     apatheme+
...
#     labs(x="Ordinal Position",y ="Normalized RTs (ms)")) +
#     facet_wrap(means_final_subject$subject)+
#     guides(color=guide_legend(title="Comparison to effect \nof ordinal position \nin the GLMM"))
#   # annotate(geom="text", x=1.5, y=1330, label="n = 30", 
#   #         color="black", size = 8))
# 
# filename <- "CSI_online_typing_by_participant_mediansplit.pdf"
# ggsave(plot_rt, filename = 
#          here::here("figures", filename),
