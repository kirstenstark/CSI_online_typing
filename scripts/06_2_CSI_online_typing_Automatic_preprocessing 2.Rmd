...
case_character_type <- function(word, item, wordcorrected, jw, bestmatch, d) {
  case_when(
    ...
    (jw < d) & (substring(wordcorrected,1,1) == substring(bestmatch,1,1)) & (substring(word, 1,1) == substring(bestmatch,1,1)) ~ "almostcorrect",
...
df2 <- df2 %>% mutate(correct_auto = case_when(answer_auto == "almostcorrect" ~ 1,
