...
Additionally, as the p-value based on the Wald-Z statistic is only partly reliable, we estimate the significance using likelikood ratio tests
...
# ---------------------------------------------
# Additional analyses
Additional models to control for covariates
...
## with category as additional random effects level
  ...
  for(j in 1:length(unique(df_full$category))){
    for(k in 2:length(unique(df_full$PosOr))){
      ...
      lag1 <- df_full$trial[df_full$subject==subjects[i] & 
                                   df_full$category == categories[j] &
                                   df_full$PosOr == k-1]
      df_full$lag[df_full$subject==subjects[i] & df_full$category == categories[j] &
...
m1_lag <- glmer(timing.01 ~ PosOr.cont*lag_cont + (PosOr.cont|subject) +
            ...
            control=glmerControl(optimizer = "bobyqa"))
summary(m1_lag)
sjPlot::tab_model(m1_lag,transform = NULL,pred.labels = c("Intercept","Ordinal Position", "Lag", "Ordinal position*Lag"),
          show.re.var = F, show.stat = T, string.stat = "t-Value", show.se = T, show.r2 = F,show.icc = F)
...
```{r}
print("Mean:"); mean(df_errors$error_sum)
print("SD:"); sd(df_errors$error_sum)
print("By-subject mean:"); mean(means_error_subject$error_sum)
print("By-subject mean:"); mean(means_error_subject$sd)
```
...
```{r plot_by_size_subcat}
modeloutput <- coef(m1_error)$category
(means_final_subcat <- df_errors %>% 
...
for(i in 1:nrow(means_final_category)) {
  means_final_category$grandmean[i] <- means_final$error[
    means_final$PosOr == means_final_subcat$PosOr[i]] -
    means_final$error[means_final$PosOr== "1"]
  means_final_category$normalizederror[i] <- means_final_category$error_sum[i] -
    means_final_category$error_sum[means_final_category$subcat ==
                             means_final_category$subcat[i] &
                             means_final_subcat$Ord.Pos == "1"]
  # prepare for ordering
...
means_final_category$effect <- as.factor(
  round(means_final_category$effect, 2))
means_final_subcat$effect <- factor(
  means_final_subcat$effect, levels=rev(levels(means_final_subcat$effect )))

# give categories English names and combine with effect size
means_final_subcat <- means_final_subcat %>% 
  mutate(subcat_en = case_when(
    subcat == "Gebaeude" ~ paste0(
      "Buildings\n\n(", effect, ")", sep=''),
    subcat == "Huftiere" ~ paste0(
      "Hoofed\nanimals\n(", effect, ")", sep=''),
    subcat == "Jacken" ~ paste0(
      "Jackets\n\n(", effect, ")", sep=''),
    subcat == "Aufbewahrung" ~ paste0(
      "Storage\n\n(", effect, ")", sep=''),
    subcat == "Instrumente" ~ paste0(
      "Instruments\n\n(", effect, ")", sep=''),
    subcat == "Blumen" ~ paste0(
      "Flowers\n\n(", effect, ")", sep=''),
    subcat == "Koerperteile" ~ paste0(
      "Body parts\n\n(", effect, ")", sep=''),
    subcat == "Obst" ~ paste0(
      "Fruits\n\n(", effect, ")", sep=''),
    subcat == "Suessigkeiten" ~ paste0(
      "Sweets\n\n(", effect, ")", sep=''),
    subcat == "Kueche" ~ paste0(
      "Kitchen\nfurniture\n(", effect, ")", sep=''),
    subcat == "Trinkgefaesse" ~ paste0(
      "Drinking\nvessels\n(", effect, ")", sep=''),
    subcat == "Voegel" ~ paste0(
      "Birds\n\n(", effect, ")", sep=''),
    subcat == "Bauernhof" ~ paste0(
      "Farming\ntools\n(", effect, ")", sep=''), 
    subcat == "Strasse" ~ paste0(
      "Street\nvehicles\n(", effect, ")", sep=''),
    subcat == "Gemuese" ~ paste0(
      "Vegetables\n\n(", effect, ")", sep=''),
    subcat == "Buero" ~ paste0(
      "Office\ntools\n(", effect, ")", sep=''),
    subcat == "Heimwerker" ~ paste0(
      "Carpenter.s\ntools\n(", effect, ")", sep=''),
    subcat == "Insekten" ~ paste0(
      "Insects\n\n(", effect, ")", sep=''),
    subcat == "Raubtiere" ~ paste0(
      "Predators\n\n(", effect, ")", sep=''),
    subcat == "Kochen" ~ paste0(
      "Cooking\nequipment\n(", effect, ")", sep=''),
    subcat == "Schmuck" ~ paste0(
      "Jewelry\n\n(", effect, ")", sep=''),
    subcat == "Sitzen" ~ paste0(
      "Seating\nfurniture\n(", effect, ")", sep=''),
    subcat == "Fische" ~ paste0(
      "Fish\n\n(", effect, ")", sep=''),
    subcat == "Wasser" ~ paste0(
      "Water\nvehicles\n(", effect, ")", sep=''))) %>%
  mutate(subcat_en = case_when(subcat_en=="Fish\n\n(0.1)" ~
                                    "Fish\n\n(0.10)",
                                  subcat_en=="Hoofed\nanimals\n(0.2)" ~
                                    "Hoofed\nanimals\n(0.20)",
                                  TRUE~subcat_en)) %>%
  mutate(subcat_en=factor(subcat_en,levels=c(
    "Buildings\n\n(0.23)","Hoofed\nanimals\n(0.20)",
    "Jackets\n\n(0.18)","Storage\n\n(0.17)",
    "Instruments\n\n(0.16)","Flowers\n\n(0.16)",
    "Body parts\n\n(0.16)","Fruits\n\n(0.16)",
...
# Plotting
(plot_error_subcat <- means_final_subcat %>% 
    ggplot(., aes(x=Ord.Pos,y=normalizederror, na.rm=T)) +
    geom_point(size =1, color = 'black') +
    geom_line(aes(x=Ord.Pos,y=normalizederror, color="a", linetype="c"),
              group = 1,size = 0.5) +
    geom_line(aes(x=Ord.Pos,y=grandmean, color="b", linetype="d"), 
              group = 1,size = 0.8)+
    ...
    apatheme+
    labs(x="Ordinal Position",y ="Normalized RTs (ms)") +
    facet_wrap(means_final_subcat$subcat_en, scales='free', ncol=6)+
...
filename <- "CSI_online_spoken_errors_by_subcat.pdf"
ggsave(plot_error_subcat, filename = 
         here::here("data", "verbal_CSI", "Plots", filename),
...
````{r plot_grid warnings=FALSE, message=FALSE, echo=FALSE}
